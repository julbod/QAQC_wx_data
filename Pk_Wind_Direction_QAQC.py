...
wx_stations = [x for x in wx_stations if not "clean_datlamen" in x] # remove legacy data for Datlamen
wx_stations = [x for x in wx_stations if not "clean_mountmaya" in x] # remove legacy data for Datlamen
wx_stations = [x for x in wx_stations if not "clean_rennellpass" in x] # remove legacy data for Datlamen
wx_stations = [x for x in wx_stations if not "clean_eastbuxton" in x] # remove legacy data for Datlamen
...
for l in range(len(wx_stations_name)): 
    ...
    # otherwise, if data (most stations), keep running
        ...
        qaqced_array = pd.concat([qaqc_arr_final['DateTime'],qaqc_arr_final['Pk_Wind_Dir'],qaqc_arr_final['Pk_Wind_Dir_flags']],axis=1)
        ...
        existing_qaqc_sql[colnames[col_positions]] = pd.concat([qaqced_array['Pk_Wind_Dir'],qaqced_array['Pk_Wind_Dir_flags']],axis=1)
