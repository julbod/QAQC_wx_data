...
for l in range(len(wx_stations_name)):
    ...
    Path("D:/Vancouver_Island_University/Wx_station/wx_data_processing/QAQC/v2/individual_figures/" + sql_database + "/Pk_Wind_Dir").mkdir(parents=True, exist_ok=True)
    os.chdir("D:/Vancouver_Island_University/Wx_station/wx_data_processing/QAQC/v2/individual_figures/" + sql_database + "/Pk_Wind_Dir")
    ...
    var = 'Pk_Wind_Dir'
    var_name = 'Peak Wind Direction'
    var_name_short = 'Pk_Wind_Dir'
    ...
    for k in range(len(yr_range)):
        ...
        # days missing at start or end of water year
            ...
            # then skip the dataset completely and go to next year
                ...
                with open("D:/Vancouver_Island_University/Wx_station/wx_data_processing/QAQC/v2/individual_figures/" + sql_database + "/Pk_Wind_Dir/messages_" + sql_name + ".csv", 'a', newline='') as f_object:
        ...
        if sql_file[var].isnull().all() or dt_yr.size == 0: # if all nans or there is no yearly data
            print('It appears that there is no Peak Wind Dir data for year: %d-%d - exiting loop' %(yr_range[k],yr_range[k]+1))
            
            # write warning to csv for inventory
            with open("D:/Vancouver_Island_University/Wx_station/wx_data_processing/QAQC/v2/individual_figures/" + sql_database + "/Pk_Wind_Dir/QAQC_issues_" + sql_name + ".csv", 'a', newline='') as f_object:
                writer_object = writer(f_object)
                writer_object.writerow([sql_name + ' ' + var_name + ': lack of Peak Wind Dir - not QA/QCed for year:', yr_range[k],yr_range[k]+1])
                f_object.close()
        ...
        #%% Remove duplicate consecutive values == 360deg or == 0deg for Peak Wind Dir
        # equal to 360 deg        
        data = qaqc_arr[var].iloc[np.arange(dt_yr[0].item(),dt_yr[1].item()+1)]
        flag = 3
        ...
        qaqc_arr['Pk_Wind_Dir_flags'] = flags.apply(qaqc_functions.merge_row, axis=1)
    ...
    # otherwise, if data (most stations), keep running
        ...
        qaqc_PkWindDir = pd.concat([qaqc_arr_final['DateTime'],qaqc_arr_final['Pk_Wind_Dir'],qaqc_arr_final['Pk_Wind_Dir_flags']],axis=1)
        ...
        existing_qaqc_sql[colnames[col_positions]] = pd.concat([qaqc_PkWindDir['Pk_Wind_Dir'],qaqc_PkWindDir['Pk_Wind_Dir_flags']],axis=1)
