...
wx_stations = [x for x in wx_stations if not "plummerhut" in x] # remove rennell from list
...
for l in range(len(wx_stations_name)):
    ...
    Path("D:/Vancouver_Island_University/Wx_station/wx_data_processing/QAQC/v2/individual_figures/" + sql_database + "/PC_Raw_Pipe").mkdir(parents=True, exist_ok=True)
    os.chdir("D:/Vancouver_Island_University/Wx_station/wx_data_processing/QAQC/v2/individual_figures/" + sql_database + "/PC_Raw_Pipe")
    ...
    for k in range(len(yr_range)):
        ...
        # days missing at start or end of water year
            ...
            # then skip the dataset completely and go to next year
                ...
                with open("D:/Vancouver_Island_University/Wx_station/wx_data_processing/QAQC/v2/individual_figures/" + sql_database + "/PC_Raw_Pipe/messages_" + sql_name + ".csv", 'a', newline='') as f_object:
        ...
        # likewise, if PC_Raw_Pipe is all NaNs or there is no yearly data
        ...
        if sql_file[var].isnull().all() or dt_yr.size == 0: # if all nans or there is no yearly data
            print('It appears that there is no precip pipe raw data for year: %d-%d - exiting loop' %(yr_range[k],yr_range[k]+1))
            
            # write warning to csv for inventory
            with open("D:/Vancouver_Island_University/Wx_station/wx_data_processing/QAQC/v2/individual_figures/" + sql_database + "/PC_Raw_Pipe/QAQC_issues_" + sql_name + ".csv", 'a', newline='') as f_object:
                writer_object = writer(f_object)
                writer_object.writerow([sql_name + ' ' + var_name + ': lack of precip pipe raw - not QA/QCed for year:', yr_range[k],yr_range[k]+1])
                f_object.close()
    ...
    # as above, skip iteration if all PC_Raw_Pipe is null
    ...
    # otherwise, if data (most stations), keep running
        ...
        qaqc_sDepth = pd.concat([qaqc_arr_final['DateTime'],qaqc_arr_final['PC_Raw_Pipe'],qaqc_arr_final['PC_Raw_Pipe_flags']],axis=1)
        ...
        existing_qaqc_sql[colnames[col_positions]] = pd.concat([qaqc_sDepth['PC_Raw_Pipe'],qaqc_sDepth['PC_Raw_Pipe_flags']],axis=1)
