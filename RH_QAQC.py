...
for l in range(len(wx_stations_name)):
    ...
    for k in range(len(yr_range)):
        ...
        # days missing at start or end of water year
            ...
            # then skip the dataset completely and go to next year
                ...
                with open("D:/Vancouver_Island_University/Wx_station/wx_data_processing/QAQC/v2/individual_figures/" + sql_database + "/RH/messages_" + sql_name + ".csv", 'a', newline='') as f_object:
        ...
        # likewise, if RH is all NaNs or there is no yearly data
        ...
        if sql_file[var].isnull().all() or dt_yr.size == 0: # if all nans or there is no yearly data
            print('It appears that there is no RH data for year: %d-%d - exiting loop' %(yr_range[k],yr_range[k]+1))
            
            # write warning to csv for inventory
            with open("D:/Vancouver_Island_University/Wx_station/wx_data_processing/QAQC/v2/individual_figures/" + sql_database + "/RH/QAQC_issues_" + sql_name + ".csv", 'a', newline='') as f_object:
                writer_object = writer(f_object)
                writer_object.writerow([sql_name + ' ' + var_name + ': lack of RH - not QA/QCed for year:', yr_range[k],yr_range[k]+1])
                f_object.close()
    ...
    # as above, skip iteration if all RH is null
    ...
    # otherwise, if data (most stations), keep running
        ...
        qaqc_RH = pd.concat([qaqc_arr_final['DateTime'],qaqc_arr_final['RH'],qaqc_arr_final['RH_flags']],axis=1)
        ...
        existing_qaqc_sql[colnames[col_positions]] = pd.concat([qaqc_RH['RH'],qaqc_RH['RH_flags']],axis=1)
