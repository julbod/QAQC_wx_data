# This code attempts to QA/QC the snow depth data in a full year for all 
# Stephanies wx stations and all years
...
wx_stations = [x for x in wx_stations if "steph" in x]
wx_stations = [x for x in wx_stations if not "_test" in x] # remove legacy data for Cairnridgerun
wx_stations = [x for x in wx_stations if not "clean_steph3" in x] # remove legacy data for Cairnridgerun\
wx_stations = [x for x in wx_stations if not "clean_steph10" in x] # remove legacy data for Cairnridgerun\
...
for l in range(len(wx_stations_name)):
    ...
    Path("D:/Vancouver_Island_University/Wx_station/wx_data_processing/QAQC/v2_stephs/individual_figures/" + sql_database + "/SnowDepth").mkdir(parents=True, exist_ok=True)
    os.chdir("D:/Vancouver_Island_University/Wx_station/wx_data_processing/QAQC/v2_stephs/individual_figures/" + sql_database + "/SnowDepth")
    ...
    if wx_stations_name[l] == 'steph1':
        yr_range = np.arange(int(yr_range[np.flatnonzero(yr_range == 2012)]),yr_range[-1]+1)
        
    if wx_stations_name[l] == 'steph4':
        yr_range = np.arange(int(yr_range[np.flatnonzero(yr_range == 2020)])+1,yr_range[-1]+1)
        
    if wx_stations_name[l] == 'steph6':
        yr_range = np.delete(yr_range, np.flatnonzero(yr_range == 2016))
        
    if wx_stations_name[l] == 'steph7':
        yr_range = np.delete(yr_range, np.flatnonzero(yr_range == 2011))
        yr_range = np.delete(yr_range, [np.flatnonzero(yr_range == 2014),np.flatnonzero(yr_range == 2015)])
        
    if wx_stations_name[l] == 'steph8':
        yr_range = np.delete(yr_range, [np.flatnonzero(yr_range == 2014),np.flatnonzero(yr_range == 2015),np.flatnonzero(yr_range == 2018),np.flatnonzero(yr_range == 2019),np.flatnonzero(yr_range == 2020),np.flatnonzero(yr_range == 2021)])
    ...
    for k in range(len(yr_range)):
        ...
        try:
            ...
            dt_summer_yr = np.concatenate(([np.where(dt_sql == np.datetime64(datetime(yr_range[k]+1, 4, 15, 00, 00, 00))), np.where(dt_sql == np.datetime64(datetime(yr_range[k]+1, 9, 23, 00, 00, 00)))]))
        ...
        # days missing at start or end of water year
            ...
            if 'dt_winter' in err_var and start_yr_sql <= datetime(yr_range[k], 12, 15, 00, 00, 00):
                ...
                dt_winter_yr = np.concatenate(([np.where(dt_sql == start_yr_sql), np.where(dt_sql == end_yr_sql)]))
                dt_summer_yr = np.concatenate(([np.where(dt_sql == np.datetime64(datetime(yr_range[k]+1, 4, 15, 00, 00, 00))), np.where(dt_sql == end_yr_sql)]))
            ...
            # then skip the dataset completely and go to next year
                ...
                with open("D:/Vancouver_Island_University/Wx_station/wx_data_processing/QAQC/v2_stephs/individual_figures/" + sql_database + "/SnowDepth/messages_" + sql_name + ".csv", 'a', newline='') as f_object:
        ...
        if sql_file[var].isnull().all() or dt_yr.size == 0: # if all nans or there is no yearly data
            ...
            # write warning to csv for inventory
            with open("D:/Vancouver_Island_University/Wx_station/wx_data_processing/QAQC/v2_stephs/individual_figures/" + sql_database + "/SnowDepth/QAQC_issues_" + sql_name + ".csv", 'a', newline='') as f_object:
                writer_object = writer(f_object)
        ...
        #idx_first = int(np.flatnonzero(qaqc_arr['DateTime'] == '2017-06-26 00:00:00'))
        #idx_last = int(np.flatnonzero(qaqc_arr['DateTime'] == '2017-09-30 23:00:00'))
        ...
        # add fix where qaqc is failing to identify outliers
        if wx_stations[l] == 'clean_steph6' and yr_range[k] == 2018:
            idx_first = int(np.flatnonzero(qaqc_arr['DateTime'] == '2018-10-08 18:00'))
            idx_last = int(np.flatnonzero(qaqc_arr['DateTime'] == '2018-10-16 14:00'))
            qaqc_arr[var].iloc[idx_first:idx_last] = np.nan
            flags_1.iloc[idx_first:idx_last] = 1
            
        if wx_stations[l] == 'clean_steph6' and yr_range[k] == 2022:
            idx_first = int(np.flatnonzero(qaqc_arr['DateTime'] == '2022-10-08 18:00'))
            idx_last = int(np.flatnonzero(qaqc_arr['DateTime'] == '2022-11-03 03:00'))
            qaqc_arr[var].iloc[idx_first:idx_last] = np.nan
            flags_1.iloc[idx_first:idx_last] = 1
            
        if wx_stations[l] == 'clean_steph7' and yr_range[k] == 2019:
            idx_first = int(np.flatnonzero(qaqc_arr['DateTime'] == '2019-10-01 00:00'))
            idx_last = int(np.flatnonzero(qaqc_arr['DateTime'] == '2019-10-28 16:00'))
            qaqc_arr[var].iloc[idx_first:idx_last] = np.nan
            flags_1.iloc[idx_first:idx_last] = 1
