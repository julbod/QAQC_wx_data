...
wx_stations = [x for x in wx_stations if not "rennell" in x] # remove rennell from list
    ...
    for k in range(len(yr_range)):
        ...
        # days missing at start or end of water year
            ...
            # then skip the dataset completely and go to next year
                ...
                with open("D:/Vancouver_Island_University/Wx_station/data/QAQC/" + sql_database + "/SnowDepth/messages_" + sql_name + ".csv", 'a', newline='') as f_object:
        ...
        if sql_file[var].isnull().all() or dt_yr.size == 0: # if all nans or there is no yearly data
            ...
            # write warning to csv for inventory
            with open("D:/Vancouver_Island_University/Wx_station/data/QAQC/" + sql_database + "/SnowDepth_v2/QAQC_issues_" + sql_name + ".csv", 'a', newline='') as f_object:
                writer_object = writer(f_object)
        ...
        plt.savefig('%d_%s %s Final Comparison WTYR %d-%d.png' %(k,sql_name,var_name_short,yr_range[k],yr_range[k]+1), dpi=400)
        plt.close()
        plt.clf()
